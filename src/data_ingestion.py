...
class DataIngestion:
    ...
    def connect_to_db(self):
        try:
            conn = psycopg2.connect(
                ...
                dbname = self.db_params['schema'],
                user = self.db_params['user'],
                password = self.db_params['password']

            )

            logger.info("Database connection has been estabilished")
            return conn
    ...
    def extract_data(self):
        try:
            ...
            conn.close()
            logger.info("Data has been successfully extracted from DB")
            return df
        except Exception as e:
            logger.error(f"Failed to extract data from DB {e}")
            raise CustomException(str(e),sys)
    ...
    def run(self):
        try:
            logger.info("Data Ingestion process has been started")
